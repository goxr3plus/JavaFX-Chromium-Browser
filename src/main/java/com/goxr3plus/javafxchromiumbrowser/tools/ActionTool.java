...
public final class ActionTool {
	...
	public static void showNotification(String title , String text , Duration d , NotificationType notificationType , ImageView imageView) {
		...
		try {
			...
			if (imageView == null)
				notification1 = Notifications.create().title(title).text(text).hideAfter(d).darkStyle().position(Pos.BOTTOM_RIGHT);
			else
				notification1 = Notifications.create().title(title).text(text).hideAfter(d).darkStyle().position(Pos.BOTTOM_RIGHT).graphic(imageView);
